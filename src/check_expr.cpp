...
gb_internal void check_format_string(Operand *fmt_operand, Slice<Operand> variadic_operands) {
	...
	for(isize i=0; i< fmt.len; i++) {
		...
		if(ops.width_arg_num){
			if(ops.width >= variadic_operands.count){
				error(fmt_operand->expr, "format string references argument %lu that doesn't exist", ops.width);
				return;
			}
			
			if (!is_type_numeric(default_type(variadic_operands[ops.width].type))) {
				error(fmt_operand->expr, "format string %lu argument is not numeric", ops.width);
				return;
			}
		}
		if(ops.precision_arg_num){
			if(ops.precision >= variadic_operands.count){
				error(fmt_operand->expr, "format string references argument %lu that doesn't exist", ops.precision);
				return;
			}
			
			if (!is_type_numeric(default_type(variadic_operands[ops.precision].type))) {
				error(fmt_operand->expr, "format string %lu argument is not numeric", ops.precision);
				return;
			}
		}
		GB_ASSERT(ops.arg_num_set);
		if(ops.arg_num >= variadic_operands.count){
			error(fmt_operand->expr, "format string references argument %lu that doesn't exist", ops.arg_num);
			return;
		}
	...
	if(__builtin_popcount(used) < variadic_operands.count ) {
		error(fmt_operand->expr, "format string uses %i arguments, but %ti supplied", __builtin_popcount(used), variadic_operands.count);
	}
}
